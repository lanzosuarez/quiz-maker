...
function createApp() {
	...
	app.post("/quizzes/:id/questions", (req, res) => {
		try {
			...
			const { type, prompt, options, correctAnswer, codeSnippet, position } = req.body || {};
			if (!type || !prompt)
			...
			const stmt =
				db.prepare(`INSERT INTO questions (quiz_id, type, prompt, options_json, correct_answer, code_snippet, position, created_at)
        VALUES (?,?,?,?,?,?,?,?)`);
			const info = stmt.run(
				quizId,
				type,
	...
	app.patch("/questions/:id", (req, res) => {
        ...
        WHERE id=?`);
			stmt.run(
				...
				codeSnippet === undefined ? null : (codeSnippet || null),
